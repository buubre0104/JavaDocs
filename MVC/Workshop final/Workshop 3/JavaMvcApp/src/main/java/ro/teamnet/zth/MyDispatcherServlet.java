        ...
        try {
            ...
            for (Parameter par : parameter) {
                if (par.isAnnotationPresent(MyRequestParam.class)) {
                   ...
                   //daca vreau id fara ghilimele...........il fac sa aleaga intre string si mapper
                    Object requestParamObject;
                    if(type.equals(String.class))
                       requestParamObject = reqParamValue;
                        else
                        requestParamObject = new ObjectMapper().readValue(reqParamValue, type);

                    parameterValues.add(requestParamObject);
                }

                else if(par.isAnnotationPresent(MyRequestObject.class)){
                    Class<?> type = par.getType();
                    BufferedReader buff = req.getReader();
                    Object reqBodyObject = new ObjectMapper().readValue(buff, type);
                    parameterValues.add(reqBodyObject);
